...
- `followup_intent`: whether the developer wants `SPEC_analyzeUserStory` on the aborted evidence or a new `SPEC_importIssue` improvement/refinement round.
...
- Next recommended command: `SPEC_analyzeUserStory` when the next work should reuse the preserved story evidence, or `SPEC_importIssue` when the problem should become a new improvement/refinement input for the next round.
...
Do not continue implementation after aborting. The next round must start from `SPEC_analyzeUserStory` or `SPEC_importIssue`.
